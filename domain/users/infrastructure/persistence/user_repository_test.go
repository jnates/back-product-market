...
func TestCreateUser(t *testing.T) {
	...
	user := &model.User{Name: "Test User", Email: "test@example.com", UserIdentifier: 1,
...
func TestCreateUserConflict(t *testing.T) {
	...
	repo := NewUserRepository(pool)
	user := &model.User{Name: "Test User", UserPassword: "plaintext"}
...
func TestCreateUserQueryError(t *testing.T) {
	...
	assert.Error(t, err)
	assert.False(t, errors.Is(err, apperrors.ErrConflict))
	assert.Nil(t, created)
}
...
func TestLoginUser(t *testing.T) {
	t.Setenv(enum.SecretKey, "test-secret")
